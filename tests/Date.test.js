...
describe('ValidatorDate', () => {
    ...
    it('should accept null if nullable is allowed', () => {
        const schema = Validator.date().nullable()
    ...
    it('should validate .required()', () => {
        const schema = Validator.date().required()
    ...
    it('should validate .min() constraint', () => {
        const schema = Validator.date().min(new Date('2024-01-01'))
    ...
    it('should skip date rules for null when nullable is allowed', () => {
        const schema = Validator.date().nullable().min(new Date('2024-01-01'))
    ...
    it('should validate .max() constraint', () => {
        const schema = Validator.date().max(new Date('2025-12-31'))
    ...
    it('should support .fn() custom rule', () => {
        const schema = Validator.date().fn((value, errors) => {
    ...
    it('should handle errors in .fn()', () => {
        const schema = Validator.date().fn(() => {
    ...
    it('should accept string date when string option is true (default)', () => {
        const schema = Validator.date({ string: true })
        expect(schema.validate('2024-03-15')).to.be.true
        expect(schema.errors).to.be.empty
    })

    it('should reject string date when string option is false', () => {
        const schema = Validator.date({ string: false })
        expect(schema.validate('2024-03-15')).to.be.false
        expect(schema.errors.some((e) => e.code === 'DATE_TYPE_ERROR')).to.be.true
    })
})
